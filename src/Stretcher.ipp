...
Internal::Stretcher<FourierKernel>::Stretcher(SampleRates sampleRates, int channelCount, int log2SynthesisHopAdjust) :
	...
	output(transforms, log2SynthesisHop, channelCount, maxOutputFrameCount(true), 0.25f, {1.f, 0.5f})
{
	for (auto &grain : grains.vector)
		grain = std::make_unique<Grain<FourierKernel>>(log2SynthesisHop, channelCount, maxInputFrameCount(true));

	Fourier::resize<true>(grains[0].log2TransformLength, 1, temporary);
}

template <class FourierKernel>
InputChunk Internal::Stretcher<FourierKernel>::specifyGrain(const Request &request, double bufferStartPosition)
{
...
template <class FourierKernel>
void Internal::Stretcher<FourierKernel>::analyseGrain(const float *data, std::ptrdiff_t stride, int muteFrameCountHead, int muteFrameCountTail)
{
...
template <class FourierKernel>
void Internal::Stretcher<FourierKernel>::synthesiseGrain(OutputChunk &outputChunk)
{
	...
	if (grain.valid())
	{
		...
		t = grain.rotation.topRows(grain.validBinCount).template cast<float>() * (std::complex<float>{0, std::numbers::pi_v<float> / 0x8000});
		t = t.exp();
	...
	Output<FourierKernel>::Segment::lapPadding(grains[3].segment, grains[2].segment);
